...
- `devloop cycle advise <cycle-id> [--role ROLE] --allow-call [--write-report] [--overwrite-report]`: runs an authorized model-backed advisory request using an OpenAI-compatible `/chat/completions` transport. Validates model config, cycle structure, policy, and explicit `--allow-call`; resolves role/model/provider; collects only allowlisted artifacts; and prints sanitized metadata plus advisory text to stdout. Default mode remains stdout-only and non-mutating. With explicit `--write-report`, it persists `.ai-loop/cycles/<cycle-id>/advisory.md`; by default it refuses to overwrite an existing advisory report, and explicit `--overwrite-report` allows replacement. `--overwrite-report` is valid only with `--write-report`. It never executes shell commands, invokes agents, persists raw payloads/responses, or prints secrets. See [Supervisor Config](docs/supervisor-config.md) for details on behavior, output shape, and safety constraints. Expected supervisor advisory sections: `Summary`, `Readiness`, `Risks`, `Recommended next microtask`, `Validation commands`, and `Open questions`.
