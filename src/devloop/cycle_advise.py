...
SYSTEM_PROMPT = (
    ...
    "`plan.md`, `prompt.md`, and `summary.md` are optional context artifacts and "
    "must not be treated as mandatory for every cycle. "
    ...
    "Treat `report.md` as a normal cycle artifact, distinct from `advisory.md`, "
    "which is the persisted model advisory file used only when "
    "`devloop cycle advise <cycle-id> --role supervisor --allow-call --write-report` "
    "is explicitly requested. "
    "When `--write-report` is used, it writes only "
    "`.ai-loop/cycles/<cycle-id>/advisory.md`; it does not write or update "
    "`report.md`, and `cycle advise` does not generate or update `plan.md`, "
    "`prompt.md`, `summary.md`, or `report.md`. "
    "If optional artifacts are absent, you may note minimal context, but do not "
    "automatically classify readiness as low only because optional artifacts are missing. "
    "For cycles focused on transport/configuration validation or advisory persistence "
    "validation, a minimal artifact set such as `meta.yaml`, `task.md`, and "
    "`report.md` may be sufficient. "
...
def _build_advisory_context(
    ...
    chunks: list[str] = [
        ...
        "optional_cycle_artifacts: plan.md; prompt.md; summary.md",
        "optional_artifact_policy: optional artifacts may be absent; absence alone does not imply low readiness",
        "minimal_validation_cycle_example: meta.yaml; task.md; report.md can be sufficient for transport/configuration or advisory persistence validation tasks",
        ...
        "advisory_persistence_contract: --write-report writes only .ai-loop/cycles/<cycle-id>/advisory.md",
        "advisory_persistence_contract: --write-report does not write or update report.md",
        "cycle_advise_mutation_contract: cycle advise does not generate or update plan.md; prompt.md; summary.md; report.md",
        "artifact_distinction: report.md is a normal cycle artifact; advisory.md is the persisted model advisory generated by --write-report",
